...
def run(process_id, args, experiment_args):
    ...
    NE_train_set = data_processing.NETrainSet(vox1, args['path_musan'], args['path_rir'], args['crop_size'])
    ...
    for kd_epoch in range(1, args['kd_epoch'] + 1):
        ...
        if int(kd_epoch % 10) == 0 and logger is not None:
            ...
            for key, v in best_state.items():
                if key == 'student':
                    logger.save_model(f'NAWSV_vox1_wavlm_params_pretrained_model_{kd_epoch}', v)
                elif key == 'classifier':
                    logger.save_model(f'NAWSV_vox1_wavlm_params_{key}_{kd_epoch}', v)

        # test
        enrollment_set_TTA.Key = 'clean'
        eer = train.val(ssl_NE_framework, enrollment_loader_TTA, vox1.trials)
