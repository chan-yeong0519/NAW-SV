...
def train_ft(epoch, framework, optimizer, loader, logger, unfreeze_ssl_epoch, masking=False):
    framework.train()
    ...
    with tqdm(total=len(loader), ncols=90) as pbar:
        for x, label in loader:
            ...
            _, loss = framework(x, label=label, unfreeze_ssl=unfreeze_ssl, masking=masking)
            ...
            if logger is not None:
                ...
                desc = f'FT-train-[{epoch}|(loss): {loss.item():.3f}'
                pbar.set_description(desc)
                pbar.update(1)
