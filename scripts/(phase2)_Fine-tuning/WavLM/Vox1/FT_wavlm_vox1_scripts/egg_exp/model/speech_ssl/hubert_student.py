...
class Custom_Hubert(nn.Module):
    def __init__(self, use_cls_token=False, adapter_hidden_size=None, mask_time_length=0, mask_time_prob=0.0, mask_feature_length=0, mask_feature_prob=0.0):
        super(Custom_Hubert, self).__init__()
        
        # set transformer encoder
        self.config = AutoConfig.from_pretrained(BASE_960)
        ...
        self.hubert = CustomHubertModel(config=self.config, use_cls_token=use_cls_token, adapter_hidden_size=adapter_hidden_size)
        
        # weight initialization
        ssl = HubertModel.from_pretrained(
            BASE_960,
        ...
        self.hubert.feature_extractor.load_state_dict(ssl.feature_extractor.state_dict(), strict=False)
        self.hubert.feature_projection.load_state_dict(ssl.feature_projection.state_dict(), strict=False)
        for i in range(self.config.num_hidden_layers):
            self.hubert.encoder.layers[i].load_state_dict(ssl.encoder.layers[i].state_dict(), strict=False)
        
    def forward(self, x, output_hidden_states=True, idx_without_adapter=None, masking=False):
        x = self.hubert(x, output_hidden_states=output_hidden_states, idx_without_adapter=idx_without_adapter, masking=masking)
...
class CustomHubertModel(HubertModel):
    def __init__(self, config, use_cls_token=False, adapter_hidden_size=None):
        ...
        if config.do_stable_layer_norm:
            self.encoder = CustomHubertEncoderStableLayerNorm(config, adapter_hidden_size=adapter_hidden_size)
        else:
            self.encoder = CustomHubertEncoder(config, adapter_hidden_size=adapter_hidden_size)
...
class CustomHubertEncoder(nn.Module):
    def __init__(self, config, adapter_hidden_size=None):
        ...
        self.layers = nn.ModuleList(
            [CustomHubertEncoderLayer(config, adapter_hidden_size=adapter_hidden_size) for i in range(config.num_hidden_layers)]
        )
...
class CustomHubertEncoderStableLayerNorm(nn.Module):
    def __init__(self, config, adapter_hidden_size=None):
        ...
        self.layers = nn.ModuleList(
            [
                CustomHubertEncoderLayerStableLayerNorm(config, adapter_hidden_size=adapter_hidden_size)
...
class CustomHubertEncoderLayer(nn.Module):
    def __init__(self, config, adapter_hidden_size=None):
        ...
        if self.use_adapter:
            self.adapter = HuBERTAdapter(config.hidden_size, adapter_hidden_size)
