...
class Custom_WavLMPlus(nn.Module):
    def __init__(self, use_cls_token=False, adapter_hidden_size=None, mask_time_length=0, mask_time_prob=0.0, mask_feature_length=0, mask_feature_prob=0.0):
        ...
        self.wavlm = CustomWavLMModel(config=config, use_cls_token=use_cls_token, adapter_hidden_size=adapter_hidden_size)
    ...
    def forward(self, x, output_hidden_states=True, idx_without_adapter=None):
        x = self.wavlm(x, output_hidden_states=output_hidden_states, idx_without_adapter=idx_without_adapter)
...
def _compute_mask_indices(
    ...
    #spec_aug_mask_idxs.extend(spec_aug_mask_idxs) 
