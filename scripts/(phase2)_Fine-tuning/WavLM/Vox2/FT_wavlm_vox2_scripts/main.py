...
def run(process_id, args, experiment_args):
    ...
    train_set = data_processing.TrainSet(vox2.train_set, args['path_musan'], args['path_rir'], args['crop_size'], args['DA_p'])
    ...
    enrollment_set_full = data_processing.FullEnrollmentSet(vox2.enrollment_set)
    ...
    #=============================================
    # model load
    ssl_path = args['path_scripts']+ f'/parameters/NAWSV_vox2_wavlm_params_pretrained_model.pt'
    ...
    for epoch in range(0, args['epoch']):
        ...
        eer = train.val(ssl_sv_framework, enrollment_loader_TTA, vox2.trials)
            ...
            if best_eer > eer:
                if logger is not None:
                    ...
                    for key, v in best_state.items():
                        if key == 'ssl':
                            logger.save_model(f'FT_vox2_wavlm_params_pretrained_model_{epoch}', v)
                        else:
                            logger.save_model(f'FT_vox2_wavlm_params_{key}_{epoch}', v)
        
        # evaluation (clean_full & noise_utt)
        if best_eer > eer and epoch > 8:
            enrollment_set_TTA.Key = 'clean'
            eer_full = train.eval(ssl_sv_framework, enrollment_loader_full, enrollment_loader_TTA, vox2.trials)
            ...
            for noise in noise_type:
                for snr in snr_type:
                    ...
                    print(set_key)
                    noise_eer = train.noise_eer(ssl_sv_framework, enrollment_loader_TTA, vox2.trials)
                    if logger is not None:
